...
export default function AlertsPage() {
  ...
  const enablePreventiveNotifications = async () => {
    ...
    localStorage.setItem("preventive-alerts-enabled", "true")
    setNotificationStatus("Preventive notifications enabled for this device.")
    new Notification("Aarogya Preventive Alerts", {
      body: "You will receive local health alerts and reminder nudges.",
    })
  }
    ...
    <div className="min-h-screen bg-black text-white flex flex-col">
      ...
      <main className="max-w-6xl mx-auto w-full px-4 py-8 flex-grow">
        ...
        <section className="mt-5">
          ...
          <button onClick={enablePreventiveNotifications} className="ml-3 px-4 py-2 bg-blue-700 text-white rounded">Enable Preventive Notifications</button>
          {notificationStatus && <p className="text-sm text-green-300 mt-2">{notificationStatus}</p>}
        </section>
      </main>
      <Footer />
    </div>
